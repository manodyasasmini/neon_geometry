 ...
 // UPDATE LOGIC
    void update() {
        if (SharedState::bossActive) {
            ...
            // PERIODIC CYCLE (4 seconds total)
            if (phaseTimer >= 4.0f) {
                phaseTimer = 0.0f;
                burstTriggered = false; // Reset trigger for next cycle
            }

            // Trigger anomaly burst at 2 seconds
            if (phaseTimer >= 2.0f && !burstTriggered) {
                burstTimer = 0.6f;
                burstTriggered = true;
            }

            // Freeze into stable square from 2.8 to 4 seconds
            if (phaseTimer >= 2.8f && phaseTimer < 4.0f) {
                stable = true;
            } else {
                stable = false;
            }
